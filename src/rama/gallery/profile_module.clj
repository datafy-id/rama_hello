...
(comment
  (->Registration #uuid "bd814b9e-6d25-4768-a8c1-962730e5e97e" "ridho" "pwdhash")
  (->ProfileEdits #uuid "bd814b9e-6d25-4768-a8c1-962730e5e97e"
                  [(display-name-edit "ridho1")
                   (pwd-hash-edit "pwdhash1")
                   (height-inches-edit 60)])
  (ProfileModule 'setup 'topologies)
  aggs/+max
  )
...
(comment
  ...
  (def _ipc (rtest/create-ipc))
  (rtest/launch-module! _ipc ProfileModule {:tasks 4 :threads 2})


  (def _module-name (get-module-name ProfileModule))
  (def _registration-depot (foreign-depot _ipc _module-name "*registration-depot"))
  (def _profile-edits-depot (foreign-depot _ipc _module-name "*profile-edits-depot"))
  (def _username->registration (foreign-pstate _ipc _module-name "$$username->registration"))
  (def _profiles (foreign-pstate _ipc _module-name "$$profiles"))

  (foreign-append! _registration-depot
                   (->Registration (str (random-uuid)) "alice" "hash1"))

  (-> _registration-depot
      (com.rpl.rama/foreign-object-info))

  (-> _registration-depot
      (com.rpl.rama/foreign-depot-partition-info 0))
